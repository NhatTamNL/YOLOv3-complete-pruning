...
class opt():
    model_def = "data_firev6/yolov3-tiny.cfg"
    ...
    for idx in CBL_idx:
        ...
        if idx in prune_idx:
            ...
            if remain == 0:
                ...
                # raise Exception
                print(f"Layer index {idx}: All channels would be pruned. Retaining one channel to avoid layer collapse.")
                mask[0] = 1  # Keep the first channel
                remain = 1
...
compact_model_name = 'weights/yolov3_fire_normal_pruning_'+str(percent)+'percent.weights'
