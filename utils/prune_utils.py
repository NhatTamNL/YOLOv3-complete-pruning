...
def init_weights_from_loose_model(compact_model, loose_model, CBL_idx, Conv_idx, CBLidx2mask):
    ...
    for idx in CBL_idx:
        ...
        if input_mask is None or input_mask.size == 0:
            print(f"Layer {idx} has no input channels after pruning.")
            continue
...
def prune_model_keep_size(model, prune_idx, CBL_idx, CBLidx2mask):
    ...
    for idx in prune_idx:
        ...
        # Two convolutional layers before the upsampling layers
        next_idx_list = [idx + 1]
        if idx == 79:
            next_idx_list.append(84)
        elif idx == 91:
        ...
        for next_idx in next_idx_list:
            next_layer = pruned_model.module_list[next_idx]
            
            # Check if the next layer is a container with Conv2d as the first element or a Conv2d directly
            if isinstance(next_layer, torch.nn.Sequential) and isinstance(next_layer[0], torch.nn.Conv2d):
                next_conv = next_layer[0]
            elif isinstance(next_layer, torch.nn.Conv2d):
                next_conv = next_layer
            else:
                print(f"Layer at index {next_idx} is not a convolutional layer, skipping.")
                continue
            ...
            if next_idx in CBL_idx:
                next_bn = next_layer[1] if isinstance(next_layer, torch.nn.Sequential) else None
                if next_bn:
                    next_bn.running_mean.data.sub_(offset)
            else:
                # For convolutional layers without BN
                next_conv.bias.data.add_(offset)
